...
.findglobalMarkers <- function(object = NULL,
                               idents = "Seurat0.6",
  ...
  tmp.all.marker <- FindAllMarkers(tmp.seurat,
...
PlotMarkerHeatmap <- function(Globalmarkers = NULL, idents = "Seurat0.6", top.gene = 50,p.adj = 0.05,scale = "row"){
  ...
  legend("topright",legend=c(unique(as.character(my.cluster$cluster))), bg="transparent",pch = rep(19,length(unique(col.data))),
         col=qualitative_hcl(length(unique(col.data)),palette = "Dynamic")[unique(my.cluster$cluster)], cex=2, box.lty=0)
